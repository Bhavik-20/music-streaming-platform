...
const EditProfileComponent = () => {
	...
	const [currentUserCookies, setCurrentUserCookies, removeCurrentUserCookies] =
		useCookies(["currentUserId"]);
	...
	const deleteAccount = async () => {
		const { paylod } = await dispatch(
			deleteProfileThunk(currentUserCookies.currentUserId)
		);
		console.log("deleted");
	...
	const SaveProfileChanges = async () => {
		try {
			const response = await dispatch(updateProfileThunk(profile));
			if (response.error) {
			...
			<div className="row">
				...
				<div className="col-10">
					<div className="w-100 h-100 flex flex-col items-center nav-bar">
						...
						<div className="col-lg-7 col-sm-10 col-10 mx-auto center-bordered-block">
							<div className="w-100 font-bold mb-4 d-flex justify-content-center">
								{/* <button
									className="back-btn col-1"
